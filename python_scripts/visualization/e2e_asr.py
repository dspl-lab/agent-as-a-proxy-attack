...
# python3 -m python_scripts.visualization.e2e_asr
import os
import re
import glob
...
def calculate_asr(dataset, threshold=3, return_samples=False):
    ...
    for sample in dataset:
        answer = re.search(
            r"<answer>\s*([^<]*?(?:\d[^<]*?))\s*(?:</answer>|(?=<answer>)|$)",  # TODO: fix this
...
def main():
    suite = "slack"
    mode = "e2e"  # e2e, extractor, evaluator
    k_values = [1, 4, 16]
    model_map = {
    ...
    for k in k_values:
        for model_name, model_id in model_map.items():
            ...
            for id_ in attack_ids:
                ...
                original_asr, e2e_asr = get_asr(
                    original_dataset_path, attack_dataset_path
                )
                original_samples, e2e_samples = get_asr(
                    original_dataset_path, attack_dataset_path, return_samples=True
                )
                print(
                    f"Model: {model_name}, k: {k}, ID: {id_}: Original ASR = {original_asr:.2f}, E2E ASR = {e2e_asr:.2f}"
                )

                original_success_by_id.append(original_samples)
                attack_success_by_id.append(e2e_samples)

                plot_data.append(
                    {"group": f"k={k}", "Model": model_name, "ASR": e2e_asr}
                )
    ...
    output_filename = f"e2e_asr_{suite}_{mode}.png"
